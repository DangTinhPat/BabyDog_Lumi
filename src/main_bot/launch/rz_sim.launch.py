...
Cap doi voi rz_real.launch.py (dung script, chi khac use_sim_time mac dinh) - noi
dung y het nhau luc moi mo (Grid trong, khong co robot) - chi khac o CHUC NANG:
ban nay dung voi `make sim`/sim.launch.py (Gazebo co /clock rieng, can use_sim_time
=true de TF khong bi lech thoi gian so voi dong ho he thong that). Chay SONG SONG
voi `make sim` o terminal khac - KHONG dung joint_state_publisher_gui gia nhu
rz.launch.py cu nua (tranh xung dot publish /joint_states voi joint_state_
broadcaster that cua sim dang chay) - chi doc /tf sim dang publish that.
"""
...
def generate_launch_description():
    ...
    declare_use_sim_time = DeclareLaunchArgument(
        ...
        description='true (mac dinh) - khop dong ho Gazebo (/clock) cua `make sim`.'
    )
